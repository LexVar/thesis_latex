...
% Every message is transmitted one block at a time, each block is 16 bytes.
...
The protocols only have one transmission. They are separated only for readability.
...
The data is verified using a hash function and the board's \ac{ECC} services.
...
Then, the device decrypts and authenticates the keys using a predefined symmetric key stored in a secure PUF slot. The protocol used is the same for secure data exchange described in \ref{chap:implementation:protocol:data-exchange}.
