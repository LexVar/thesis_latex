...
Firstly it presents the authentication, then main services, divided in their objective, secure data exchange or new communications. %% ADD DIGITAL SIGNATURES REFERENCE
...
% The protocols are composed of one transmission. Messages are separated for readability.
...
% The hash is generated from the data. The result is compared with the value obtained from the decryption of the signature using the signer's public key. The signature is verified if the values are identical.
