...
All implemented services from chapter \ref{chap:implementation} were tested from a data size of 0.5 KB to 20 KB. Each service performance depends on the used accelerator's, among other factors such as writing, access to memory or other implemented logic.
Four implemented services were tested, encryption and authentication, decryption and authentication, import keys service, all three using the \ac{AES} and \ac{HMAC} accelerators, \ac{ECDSA} and \ac{ECDH}, both using the \ac{ECC} scalar multiplication accelerator and a \ac{SHA} accelerator.
Similarly to the previous core tests, the time results evolve linearly and the equation components for each service were calculated using the linear regression algorithm. The values are presented in table \ref{tab:services-model}.

\input{tables_and_code/services-model.tex}

Analyzing the results, both encryption/decryption and authentication services predictably have very similar values, due to being based on the same board's accelerators and \ac{AES} encryption and decryption having no discernible performance difference. The data dependent value is identical to the sum of both \ac{AES} and \ac{HMAC} values, \(11.124+7.815\approx18.872\).
For both services the results were calculated from the 5 KB data size to 20 KB. This is due to outlier values obtained from 0.5 KB to 5 KB, in those size ranges each operation time was constant at 128 ms for every data size value. 
The import keys service results in odd values, most likely for the same unknown reasons of the previous services outlier values, noting that this service was only tested up to 5 KB of data. This limit is due to the \ac{RAM} limitations of the \ac{HSM}, more demanding than other operations to key storage in memory.
...
We can also conclude the measured models are a good representation of the performance, all median average error percentages are below 0.4\%.
...
Figure \ref{fig:performance:services-tput} plots the results for throughput (KB/s) for the implemented services. Encrypt/decryption + MAC services' throughput increases and stabilizes at around 50 Kb/s, while \ac{ECDSA} and key importation present linear scalability.
