...
Thus, it is a relatively low cost board, which can be distributed among several individuals or groups, and has the necessary functionalities to secure their communications.
All the developed security services are performed inside the device, with its functionalities.
The focus of the developed prototype is the secure data service, which provides confidentiality and authentication to communications. Data is secured with internal keys, which are securely stored inside the device.
...
Whenever a tamper attempt or error is detected, the device is blocked and does not allow commands, and eventually is completely erased using its functions. Any potential attacks and data leaks data are prevented. This can have a denial of service effect for users, but is deemed a necessary compromise for production use.
A PKCS\#11 API was developed to expose the device's features and increase device interoperability.
