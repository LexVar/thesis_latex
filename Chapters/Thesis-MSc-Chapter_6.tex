...
The performance can be modelled by a linear equation with values in seconds, \(T_{total} = 0.0073 + 0.0886 * KB\), with a median average percentage error 0.92 \%.
...
% Dois bibliotecas foram usadas, uma para AES e a outra para SHA-256 e HMAC-SHA-256. Esta ultima foi a mesma utilizada para computar hmac continuamente no serviços de secure comms.
% https://github.com/intel/tinycrypt
% Mbedtls foi experimentada, retirando todas os algoritmos desnecessários mas ocupada demasiado espaço para incluir na placa limitada a 64 KB de RAM.
...
% SHA
% Os resultados de HMAC e SHA é esperado que seja mt similar devido ao HMAC ser baseado em 2 operações de sha. Olhando para os resultados de cores, são estranhos devido ao desempenho consideralvemente mais lento do HMAC comparado a SHA.
% Comparado aos resultados com software, no qual HMAC é neglegivelmente mais lento, HMAC e SHA tem performance sobreposta, um pouco mais lento que o core de SHA mas consideravelmente mais rápido do que HMAC.
% Não encontrei nenhuma explicação razoável para esta imensa descrepancia. Uma das razões que pode contribuir é que a implementalão de HMAC é resistente a ataques diferenciais DPA, embora isso n explica a descrepancia com a performance do core de SHA.
