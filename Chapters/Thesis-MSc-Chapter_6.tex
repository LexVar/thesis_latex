...
This section evaluates the fulfilled requirements, defined in chapter \ref{chap:problem:requirements}, by the developed system.
Firstly, the chosen device smartfusion2 SoC is a portable board, with a robust set of security services.
Compared to the existing HSMs on the market, this device is one of the cheapest. A M2S090TS smartfusion2 evaluation kit is priced at 384\€ \cite{smartfusionPrice}.
Regarding its capabilities, it provides encryption and authentication implementations, \ac{AES} and \ac{HMAC}, a secure storage service for keys, SRAM-PUF, and \ac{ECC} primitives which can be used to implement digital signatures or new key generation with \ac{ECDSA} and \ac{ECDH}. Additionally, it provides tamper detection capabilities and data zeroization.
Thus, it is a relatively low cost board, which can be distributed among several individuals or groups, and has the necessary functionalities to secure communications between them.
All the developed security services are performed inside the device, using its functionalities.
The focus of the developed prototype is the secure data service, which provides confidentiality and authentication to user's communications. Data is secured with internal keys which are securely stored in the device.
...
Users must authenticate themselves to the device, using its authentication PIN. This way entities can manage which users can use the device, in behalf of the entity.
Whenever a tamper attempt or error is detected the device is blocked, and eventually completely erased using the device's functions. This way any potential attacks and data leaks data are prevented. This can have a denial of service effect against users, but is deemed a necessary compromise.
A PKCS\#11 API was developed to access the device's features and increase device interoperability.
For users, an easy-to-use command line interface application, which calls the implemented PKCS\#11 API, was implemented.
